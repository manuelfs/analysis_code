...
int main(){ 
  ...
  TChain c_dy("tree");  c_dy.Add("/net/cms2/cms2r0/babymaker/babies/2016_06_14/mc/unskimmed/fullbaby_DYJetsToLL_M-50_TuneCU*");
  TChain c_tt("tree");  c_tt.Add("/net/cms2/cms2r0/babymaker/babies/2016_06_14/mc/unskimmed/fullbaby_TTJets_TuneCU*");
  ...
  PlotTurnOn(&c_tt, "met", nbins,minx,maxx, "E_{T}^{miss}",
	     "1", "met_tru>150",
	     "Inclusive t#bar{t}", "E_{T}^{miss tru} > 150 GeV",150, 1,false,true);

  minx = 400; maxx = 3550; nbins = static_cast<int>((maxx-minx)/50);
  PlotTurnOn(&c_tt, "ht", nbins,minx,maxx, "H_{T}",
	     "1", "ht_isr_me>600",
	     "Inclusive t#bar{t}", "H_{T}^{tru ISR} > 600 GeV",700, 1,false,true);

  minx = 0; maxx = 675; nbins = static_cast<int>((maxx-minx)/25);
  PlotTurnOn(&c_dy, "ht", nbins,minx,maxx, "H_{T}",
	     "1", "ht_isr_me>100",
	     "Inclusive Drell-Yan", "H_{T}^{tru ISR} > 100 GeV",175, 1,false,true);
...
void PlotTurnOn(TChain *data, TString var, int nbins, double minx, double maxx, TString xtitle, 
  ...
  // else label.DrawLatex(0.85, 0.93, "Inclusive Drell-Yan");
  // else label.DrawLatex(0.85, 0.93, "Inclusive t#bar{t}");
