...
int main(){ 
  ...
  TString folder(bfolder+"/cms29r0/babymaker/babies/2016_11_08/data/");
  ...
  else{c_ht.Add(folder+"/merged_database_ne1nj3_ht_jet_trig/*.root");
    c_ht.Add(folder+"/merged_database_nm1nj3_ht_jet_trig/*.root");}
  ...
  ////////////////////////////////////////////////////////////////////////////////////////////////////
  // MET100: Muon
  TString basecuts="st>500&&njets>=3"; 
  if(jet450) basecuts= "(trig[56]||trig[57])&&st>500&&njets>=3";
  ...
  vector<TString> title({"HT[200-800], N_{#mu}=1, N_{j}#geq3, S_{T}>500", "HT[200-800], N_{e}=1, N_{j}#geq3, S_{T}>500"});
  if(jet450) title ={"Jet450, N_{#mu}=1, N_{j}#geq3, S_{T}>500", "Jet450, N_{e}=1, N_{j}#geq3, S_{T}>500"};
  vector<TString> ytitle({"RA4 triggers", "RA4 triggers"});
  vector<TString> variable({"Max$(mus_pt*(mus_miniso<0.2&&mus_sigid))", "Max$(els_pt*(els_miniso<0.1&&els_sigid))","met","njets","mj14","nbm"});
  vector<TString> nminus1({"&&njets>=5&&nbm>=1&&met>100", "&&njets>=5&&nbm>=1&&met>100","&&njets>=5&&nbm>=1","&&met>100&&nbm>=1","&&njets>=5&&nbm>=1&&met>100","&&njets>=5&&met>100"});
  // vector<TString> title_nminus1({"&&met>200", "&&met>200","&&met>200","&&met>200","&&met>200","&&met>200"});
  vector<TString> lepton({"Muon","Electron"});
  vector<TString> varname({"Muon p_{T}", "Electron p_{T}","MET","N_{jets}","M_{J}","N_{b}"});
  vector<float> v_nbins_factor({5,5,25,1,24,1});
  vector<float> v_minx({20,20,0,3,0,0});
  vector<float> v_maxx({140,140,425,10,600,4});
  vector<float> v_minfit({-40,-40,-200,-5,-250,-1});
  ...
  */
  ...
  for(size_t ivar=0; ivar<variable.size(); ivar++){
	...
	minx=static_cast<int>(v_minx[ivar]);
	maxx=static_cast<int>(v_maxx[ivar]);
	...
	PlotTurnOn(&c_ht,variable[ivar],nbins,minx,maxx, varname[ivar], baseline[ilep]+runrange[irun], "trig_met", title[ilep], 
...
TString PlotTurnOn(TChain *data, TString var, int nbins, double minx, double maxx, TString xtitle, 
  ...
  TString lumi = "36.2 fb^{-1}";// "17.1 fb^{-1}";
