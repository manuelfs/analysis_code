...
namespace{
  ...
  vector<TString> startCut = {"0","10","20","30","40"};
  vector<TString> endCut   = {"10","20","30","40","50"};

  vector<double> x_pts   = {5, 15, 25, 35, 45};
  vector<double> x_start = {2.5, 2.5, 2.5, 2.5, 2.5};
  vector<double> x_end   = {2.5, 2.5, 2.5, 2.5, 2.5};

  // The "12" correponds to the number of kappa bins. As of now, needs to be changed byhand.
  vector<vector< double> > y_pts(12, vector<double>(startCut.size()));
  vector<vector< double> > y_start(12, vector<double>(startCut.size()));
  vector<vector< double> > y_end(12, vector<double>(startCut.size()));
}
...
void kappa(TString basecut, map<TString, vector<bcut> > &cutmap, vector<vector<unsigned> > &m3_njbin_ind, vector<double> const (&yield)[NSAM], vector<double> const (&w2)[NSAM], size_t ini, size_t fin, TString tag, int idx);
...
int main(){ 
  ...
  time(&begtime);
  TString folder="/cms2r0/babymaker/babies/2016_06_14/mc/merged_baseline/";
  ...
  for(unsigned int idx=0; idx<startCut.size(); idx++){
    ////// Defining cuts
    bcut baseline("nleps==1&&ht>500&&met>200&&njets>="+basenj+"&&nbm>=1&&mj14>"+lowmj+"&&ntrupv>="+startCut[idx]+"&&ntrupv<"+endCut[idx]+"&&pass&&stitch");
    ...
    if (do_metbins){
      cutmap["met"].push_back(bcut("met>"+lowmet+"&&met<="+medmet));
    ...
    if (!do_data && do_metbins) kappa(baseline.cuts_, cutmap, m3_njbin_ind, yield, w2, ini, fin, "_pv_"+startCut[idx]+"_"+endCut[idx], idx);
  ...
  for(auto iplot=0; iplot<12; iplot++){
    
    TCanvas can_plot;
    
    TH1D histo("histo","",50, 0, 50);
    histo.GetYaxis()->CenterTitle(true);
    histo.GetYaxis()->SetTitleOffset(1.2);
    histo.GetYaxis()->SetTitleSize(0.08);
    histo.SetYTitle("#kappa");
    histo.SetMaximum(2.1);
    histo.Draw();
    ...
    TGraphAsymmErrors graph = TGraphAsymmErrors(x_pts.size(), &(x_pts[0]), &(y_pts[iplot][0]), 
						&(x_start[0]), &(x_end[0]), &(y_start[iplot][0]), &(y_end[iplot][0]));
    
    graph.SetMarkerStyle(20); graph.SetMarkerSize(1.65); 
    graph.SetMarkerColor(kBlack); graph.SetLineColor(kBlack); graph.SetLineWidth(2);
    graph.Draw("p same");
    
    TString pname = "plots/kappa_pustudy_bin"+to_string(iplot);
    if(only_tt) pname += "_tt";
    else if(only_other) pname += "_other";
    else pname += "_allmc";
    pname+=".pdf";
    can_plot.SaveAs(pname);
  }
}
...
void kappa(TString basecut, map<TString, vector<bcut> > &cutmap, vector<vector<unsigned> > &m3_njbin_ind, vector<double> const (&yield)[NSAM], vector<double> const (&w2)[NSAM], size_t ini, size_t fin, TString tag, int idx){
  ...
  for(unsigned imet(0); imet<cutmap["met"].size(); imet++){
    for(unsigned inb(0); inb<cutmap["nb"].size(); inb++){
	  ...
	  double kstat = (mSigma+pSigma)/2.;
          ...
          y_pts[idxbin][idx] = kappa;    y_start[idxbin][idx] = mSigma;   y_end[idxbin][idx] = pSigma;
	  idxbin++;
  ...
  for(unsigned inj(0); inj<m3_njbin_ind.size(); inj++)
    for(unsigned imet(0); imet<cutmap["met"].size(); imet++){
      ...
      mettitle.ReplaceAll("MET > "+lowmet+", MET #leq "+medmet, lowmet+" < MET #leq "+medmet);
