...
int main(){ 
  ...
  TChain c_el("tree"); c_el.Add(folder+"/merged_higdata_ne1nj3met150/*.root"); 
  ...
  bool do_table = true;
  bool do_met = true, do_elel = false, do_mumu = false, do_el = false, do_mu = false;
  bool do_truemet = true;
  bool do_syst = false, do_mc = false;
  ...
  if(do_mc){
    ...
    PlotTurnOn(&c_el, "met", nbins,minx,maxx, "E_{T}^{miss}", "trig[40]&&"+cuts, "(trig[30] || trig[31])",  
  ...
  if(do_syst){
    ...
    vector<TString> metbins({"150", "160", "180", "200", "225", "250", "300", "9999"});
    //vector<TString> metbins({"150", "9999"});
	...
	for(unsigned ivar=0; ivar<varbins[scen].size(); ivar++){
	  if(scen==2 && ivar==0) trigger = "(trig[30] || trig[31])";
	  if(scen==2 && ivar==1) {
	    pchain = &c_mc;
	    trigger = "(trig[3] || trig[7])";
	  } else pchain = &c_el;
	  Efficiency(pchain, cuts+"&&"+varbins[scen][ivar]+"&&met"+">"+metbins[imet]+"&&met"+"<="+metbins[imet+1], 
    ...
    for(unsigned imet=0; imet<metbins.size()-1; imet++) cout<<"c";
    cout<<"}\n \\hline\\hline \n $E_{\\rm T}^{\\rm miss}$ range";
    for(unsigned imet=0; imet<metbins.size()-1; imet++) {
      cout<<" & ["<<metbins[imet];
      if(imet < metbins.size()-2) cout<<","<<metbins[imet+1];
      else cout<<"+";
      cout<<"]";
    } // mets
    cout<<"\\\\"<<endl; 
    ...
    for(unsigned scen=0; scen<varbins.size(); scen++){
      cout<<" \\hline \\hline \n \\multicolumn{"<<metbins.size()<<"}{c}{"<<syst_names[scen]<<"}\\\\ \\hline \n";
      systs.push_back(vector<float>(metbins.size(), 0));
      for(unsigned ivar=0; ivar<varbins[scen].size(); ivar++){
