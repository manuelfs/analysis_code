...
int main(){ 
  ...
  folder = bfolder+"/cms2r0/babymaker/babies/2016_08_10/data/merged/";
  // TChain ce_met("tree"); ce_met.Add(folder+"/*MET*.root");
  // TChain ce_mu("tree");  ce_mu.Add(folder+"/*Muon*root");
  // TChain ce_el("tree");  ce_el.Add(folder+"/*Electron*root");

  // TChain c_met("tree"); c_met.Add(folder+"/*MET*.root");
  // TChain c_mu("tree");  c_mu.Add(folder+"/*Muon*root");
  // TChain c_el("tree");  c_el.Add(folder+"/*Electron*root");
  ...
  for(size_t ipv=0; ipv<npv.size(); ipv++){
    ...
    PlotTurnOn(&c_el, "met", nbins,minx,maxx, "E_{T}^{miss}",
	       "(trig[22]||trig[23])&&ht>400&&Max$(els_pt*(els_miniso<0.1&&els_sigid))>25&&njets>=3&&ht>500&&nvmus==0"+npv[ipv],"(trig[13]||trig[33])",      
	       "Ele25||Ele27, N_{e}#geq1, N_{jet}#geq3, H_{T}>400", "MET100 || METNoMu100");

    // MET110 or METNoMu110
    PlotTurnOn(&c_el, "met", nbins,minx,maxx, "E_{T}^{miss}",
	       "(trig[22]||trig[23])&&ht>400&&Max$(els_pt*(els_miniso<0.1&&els_sigid))>25&&njets>=3&&ht>500&&nvmus==0"+npv[ipv],"(trig[14]||trig[15])",      
	       "Ele25||Ele27, N_{e}#geq1, N_{jet}#geq3, H_{T}>400", "MET110 || METNoMu110");

    // HT350_MET100: MET
    PlotTurnOn(&c_el, "met", nbins,minx,maxx, "E_{T}^{miss}",
	       "(trig[22]||trig[23])&&onht>400&&ht>400&&Max$(els_pt*(els_miniso<0.1&&els_sigid))>25&&njets>=3&&ht>500&&nvmus==0"+npv[ipv],"(trig[0])",      
	       "Ele25||Ele27, N_{e}#geq1, N_{jet}#geq3, H_{T}>400", "HT300_MET100");
    ...
    minx = 10; maxx = 70; nbins = static_cast<int>((maxx-minx)/5);
    PlotTurnOn(&c_met, "Max$(mus_pt*(mus_sigid&&mus_miniso<0.2))", nbins,minx,maxx, "Muon p_{T}",
	       "trig[33]&&onht>400&&ht>450&&njets>=2&&met>"+metcut+npv[ipv],"trig[3]||trig[4]",
    ...
    PlotTurnOn(&c_met, "Max$(els_pt*(els_sigid&&els_miniso<0.1))", nbins,minx,maxx, "Electron p_{T}",
	       "trig[33]&&onht>400&&ht>450&&njets>=2&&met>"+metcut+npv[ipv],"trig[7]||trig[8]",
	       "MET100, H_{T}>450, N_{jet}#geq2, MET>"+metcut, "Ele15_HT400",-1,1,true,false);


    minx = 0; maxx = 600; nbins = static_cast<int>((maxx-minx)/100);
    PlotTurnOn(&c_met, "Max$(mus_pt*(mus_sigid&&mus_miniso<0.2))", nbins,minx,maxx, "Muon p_{T}",
	       "trig[33]&&onht>400&&ht>450&&njets>=2&&met>"+metcut+npv[ipv],"trig[3]||trig[4]",
	       "MET100, H_{T}>450, N_{jet}#geq2, MET>"+metcut, "Mu15_HT400",-20,1,true,false);
    // Electron, VVVL, fine binning
    PlotTurnOn(&c_met, "Max$(els_pt*(els_sigid&&els_miniso<0.1))", nbins,minx,maxx, "Electron p_{T}",
	       "trig[33]&&onht>400&&ht>450&&njets>=2&&met>"+metcut+npv[ipv],"trig[7]||trig[8]",
	       "MET100, H_{T}>450, N_{jet}#geq2, MET>"+metcut, "Ele15_HT400",-20,1,true,false);
    ...
    // VVVL: HT400
    minx = 240; maxx = 840; nbins = static_cast<int>((maxx-minx)/20);
    PlotTurnOn(&c_met, "ht", nbins,minx,maxx, "H_{T}",
	       "trig[13]&&met>200&&njets>=2&&Max$(els_vvvl)&&nvels>=1&&run>=274960"+npv[ipv],"trig[7]",
	       "1.5fbMET100, N_{e}#geq1, N_{jet}#geq2, MET>200", "Ele15_HT400", 300,1,true,false);
    PlotTurnOn(&c_met, "ht", nbins,minx,maxx, "H_{T}",
	       "trig[13]&&Max$(mus_vvvl)&&met>200&&njets>=3&&nvmus>=1&&run>=274960"+npv[ipv],"trig[3]",
	       "1.5fbMET100, N_{#mu}#geq1, N_{jet}#geq3, MET>200", "Mu15_HT400", 300,1,true,false);
    ...
    PlotTurnOn(&c_met, "ht", nbins,minx,maxx, "H_{T}",
	       "trig[13]&&nvleps==0&&met>200&&njets>=2"+npv[ipv],"trig[0]",
