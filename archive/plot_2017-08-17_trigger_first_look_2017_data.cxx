...
int main(){ 
  ...
  TString folder(bfolder+"/cms2r0/babymaker/babies/2017_08_14/data/");
  ...
  minx = 10; maxx = 36; nbins = static_cast<int>((maxx-minx)/1);
  PlotTurnOn(&c_met, "Max$(mus_pt*(mus_sigid&&mus_miniso<0.2))", nbins,minx,maxx, "Muon p_{T}",
	     trigmet+"&&ht>550&&run>="+runvvvl+"&&njets>=1&&met>"+metcut, "trig[3]",
	     "3.77fbMET120, H_{T}>550, N_{jet}#geq1, MET>"+metcut, "Mu15_HT450",-1,1,true,false);
  // Electron, VVVL, fine binning
  PlotTurnOn(&c_met, "Max$(els_pt*(els_sigid&&els_miniso<0.1))", nbins,minx,maxx, "Electron p_{T}",
	     trigmet+"&&ht>550&&run>="+runvvvl+"&&njets>=1&&met>"+metcut, "trig[7]",
	     "3.77fbMET120, H_{T}>550, N_{jet}#geq1, MET>"+metcut, "Ele15_HT450",-1,1,true,false);

  minx = 20; maxx = 65; nbins = static_cast<int>((maxx-minx)/1);
  // Muon, Isomu27, fine binning
  PlotTurnOn(&c_met, "Max$(mus_pt*(mus_sigid&&mus_miniso<0.2))", nbins,minx,maxx, "Muon p_{T}",
	     trigmet+"&&njets>=1&&met>"+metcut, "trig[20]",
	     "MET120, N_{jet}#geq1, MET>"+metcut, "IsoMu27",26,1,true,false);
  // Electron, Ele35, fine binning
  PlotTurnOn(&c_met, "Max$(els_pt*(els_sigid&&els_miniso<0.1))", nbins,minx,maxx, "Electron p_{T}",
	     trigmet+"&&njets>=1&&met>"+metcut, "trig[23]",
	     "MET120, N_{jet}#geq1, MET>"+metcut, "Ele35_WPTight",34,1,true,false);


  minx = 10; maxx = 100; nbins = static_cast<int>((maxx-minx)/2);
  // Muon, Mu50, broad binning
  PlotTurnOn(&c_met, "Max$(mus_pt*(mus_sigid&&mus_miniso<0.2))", nbins,minx,maxx, "Muon p_{T}",
	     trigmet+"&&njets>=1&&met>"+metcut, "trig[21]",
	     "MET120, N_{jet}#geq1, MET>"+metcut, "Mu50",48,1,true,false);
  minx = 90; maxx = 160; nbins = static_cast<int>((maxx-minx)/2);
  // Electron, Ele115, broad binning
  PlotTurnOn(&c_met, "Max$(els_pt*(els_sigid&&els_miniso<0.1))", nbins,minx,maxx, "Electron p_{T}",
	     trigmet+"&&njets>=1&&run>="+runvvvl+"&&met>"+metcut, "trig[24]",
	     "3.77fbMET120, N_{jet}#geq1, MET>"+metcut, "Ele115",115,1,true,false);


  minx = 0; maxx = 750; nbins = static_cast<int>((maxx-minx)/50);
 // Muon, VVVL, broad binning
  PlotTurnOn(&c_met, "Max$(mus_pt*(mus_sigid&&mus_miniso<0.2))", nbins,minx,maxx, "Muon p_{T}",
	     trigmet+"&&ht>550&&run>="+runvvvl+"&&njets>=1&&met>"+metcut, "trig[3]",
	     "3.77fbMET120, H_{T}>550, N_{jet}#geq1, MET>"+metcut, "Mu15_HT450",-20,1,true,false);
  // Electron, VVVL, broad binning
  PlotTurnOn(&c_met, "Max$(els_pt*(els_sigid&&els_miniso<0.1))", nbins,minx,maxx, "Electron p_{T}",
	     trigmet+"&&ht>550&&run>="+runvvvl+"&&njets>=1&&met>"+metcut, "trig[7]",
	     "3.77fbMET120, H_{T}>550, N_{jet}#geq1, MET>"+metcut, "Ele15_HT450",-20,1,true,false);
  // Muon, Isomu27, broad binning
  PlotTurnOn(&c_met, "Max$(mus_pt*(mus_sigid&&mus_miniso<0.2))", nbins,minx,maxx, "Muon p_{T}",
	     trigmet+"&&njets>=1&&met>"+metcut, "trig[20]",
	     "MET120, N_{jet}#geq1, MET>"+metcut, "IsoMu27",-35,1,true,false);
  // Electron, Ele35, broad binning
  PlotTurnOn(&c_met, "Max$(els_pt*(els_sigid&&els_miniso<0.1))", nbins,minx,maxx, "Electron p_{T}",
	     trigmet+"&&njets>=1&&met>"+metcut, "trig[23]",
	     "MET120, N_{jet}#geq1, MET>"+metcut, "Ele35_WPTight",-45,1,true,false);
  // Muon, Mu50, broad binning
  PlotTurnOn(&c_met, "Max$(mus_pt*(mus_sigid&&mus_miniso<0.2))", nbins,minx,maxx, "Muon p_{T}",
	     trigmet+"&&njets>=1&&met>"+metcut, "trig[21]",
	     "MET120, N_{jet}#geq1, MET>"+metcut, "Mu50",-55,1,true,false);
  // Electron, Ele115, broad binning
  PlotTurnOn(&c_met, "Max$(els_pt*(els_sigid&&els_miniso<0.1))", nbins,minx,maxx, "Electron p_{T}",
	     trigmet+"&&njets>=1&&run>="+runvvvl+"&&met>"+metcut, "trig[24]",
	     "3.77fbMET120, N_{jet}#geq1, MET>"+metcut, "Ele115",-130,1,true,false);
  ...
  minx = 0; maxx = 470; nbins = static_cast<int>((maxx-minx)/10);
  // MET120 or METNoMu120
  PlotTurnOn(&c_mu, "met", nbins,minx,maxx, "E_{T}^{miss}",
	     "trig[20]&&Max$(mus_pt*(mus_miniso<0.2&&mus_sigid))>25&&njets>=4&&ht>500", "(trig[9])",  
	     "IsoMu27, N_{#mu,25} #geq 1, N_{jet} #geq 4", "MET120",100);
  PlotTurnOn(&c_mu, "met", nbins,minx,maxx, "E_{T}^{miss}",
	     "trig[20]&&Max$(mus_pt*(mus_miniso<0.2&&mus_sigid))>25&&njets>=4&&ht>500", "(trig[15])",  
	     "IsoMu27, N_{#mu,25} #geq 1, N_{jet} #geq 4", "METNoMu120",100);
  PlotTurnOn(&c_mu, "met", nbins,minx,maxx, "E_{T}^{miss}",
  ...
  // MET120 or METNoMu120
  PlotTurnOn(&c_el, "met", nbins,minx,maxx, "E_{T}^{miss}",
	     "trig[23]&&Max$(els_pt*(els_miniso<0.1&&els_sigid))>35&&njets>=3&&ht>500&&nvmus==0", "(trig[9]||trig[15])",      
	     "Ele35, N_{e,35} #geq 1, N_{jet} #geq 3", "MET120 || METNoMu120");
  PlotTurnOn(&c_el, "met", nbins,minx,maxx, "E_{T}^{miss}",
	     "trig[23]&&Max$(els_pt*(els_miniso<0.1&&els_sigid))>35&&njets>=3&&ht>500&&nvmus==0", "(trig[15])",      
	     "Ele35, N_{e,35} #geq 1, N_{jet} #geq 3", "METNoMu120");
  PlotTurnOn(&c_el, "met", nbins,minx,maxx, "E_{T}^{miss}",
	     "trig[23]&&Max$(els_pt*(els_miniso<0.1&&els_sigid))>35&&njets>=3&&ht>500&&nvmus==0", "(trig[9])",      
	     "Ele35, N_{e,35} #geq 1, N_{jet} #geq 3", "MET120");
  ...
  // VVVL: HT400
  minx = 240; maxx = 830; nbins = static_cast<int>((maxx-minx)/20);
  PlotTurnOn(&c_met, "ht", nbins,minx,maxx, "H_{T}",
	     //"trig[9]&&met>200&&run>="+runvvvl+"&&njets>=4&&Max$(els_vvvl)&&nvels>=1", "trig[7]",
	     "trig[9]&&met>200&&run>="+runvvvl+"&&njets>=4&&nels>=1", "trig[7]",
	     "3.77fbMET120, N_{e}#geq1, N_{jet}#geq4, MET>200", "Ele15_HT450", 400,1,true,false);
  PlotTurnOn(&c_met, "ht", nbins,minx,maxx, "H_{T}",
	     //"trig[9]&&Max$(mus_vvvl)&&met>200&&njets>=4&&nvmus>=1", "trig[3]",
	     "trig[9]&&met>200&&run>="+runvvvl+"&&njets>=4&&nmus>=1", "trig[3]",
	     "3.77fbMET120, N_{#mu}#geq1, N_{jet}#geq4, MET>200", "Mu15_HT450", 400,1,true,false);

  // HT1050: HT
  minx = 700; maxx = 1850; nbins = static_cast<int>((maxx-minx)/25);
  PlotTurnOn(&c_met, "ht", nbins,minx,maxx, "H_{T}",
	     "trig[9]&&met>200&&njets>=4&&nvleps==0", "trig[18]",
	     "MET120, N_{e,10}=0, N_{jet}#geq2, MET>200", "HT1050", 1000,1,true,false);
  PlotTurnOn(&c_met, "ht", nbins,minx,maxx, "H_{T}",
	     "trig[9]&&met>200&&run>="+runjec+"&&njets>=4&&nvleps==0", "trig[18]",
	     "No JEC bug 2.90fbMET120, N_{e,10}=0, N_{jet}#geq2, MET>200", "HT1050", 1000,1,true,false);
  PlotTurnOn(&c_met, "ht", nbins,minx,maxx, "H_{T}",
	     "trig[9]&&met>200&&run<"+runjec+"&&njets>=4&&nvleps==0", "trig[18]",
	     "With JEC bug 5.42fbMET120, N_{e,10}=0, N_{jet}#geq2, MET>200", "HT1050", 1000,1,true,false);

  // HT500_MET100: HT
  minx = 280; maxx = 960; nbins = static_cast<int>((maxx-minx)/10);
  PlotTurnOn(&c_met, "ht", nbins,minx,maxx, "H_{T}",
	     "trig[9]&&nvleps==0&&met>200&&njets>=4", "trig[0]",
	     "MET120, N_{e,10}=0, N_{jet}#geq2, MET>200", "HT500_MET100",500,1,true,false);
  PlotTurnOn(&c_met, "ht", nbins,minx,maxx, "H_{T}",
	     "trig[9]&&nvleps==0&&met>200&&run>="+runjec+"&&njets>=4", "trig[0]",
  ...
  PlotTurnOn(&c_met, "ht", nbins,minx,maxx, "H_{T}",
	     "trig[9]&&nvleps==0&&met>200&&run>="+runjec+"&&njets>=4", "trig[16]",
	     "No JEC bug 2.90fbMET120, N_{e,10}=0, N_{jet}#geq2, MET>200", "HT700_MET85",700,1,true,false);
  PlotTurnOn(&c_met, "ht", nbins,minx,maxx, "H_{T}",
	     "trig[9]&&nvleps==0&&met>200&&run<"+runjec+"&&njets>=4", "trig[16]",
	     "With JEC bug 5.42fbMET120, N_{e,10}=0, N_{jet}#geq2, MET>200", "HT700_MET85",700,1,true,false);
  // HT800_MET75: HT
  minx = 580; maxx = 1260; nbins = static_cast<int>((maxx-minx)/10);
  PlotTurnOn(&c_met, "ht", nbins,minx,maxx, "H_{T}",
	     "trig[9]&&nvleps==0&&met>200&&run>="+runjec+"&&njets>=4", "trig[17]",
	     "No JEC bug 2.90fbMET120, N_{e,10}=0, N_{jet}#geq2, MET>200", "HT800_MET75",800,1,true,false);
  PlotTurnOn(&c_met, "ht", nbins,minx,maxx, "H_{T}",
	     "trig[9]&&nvleps==0&&met>200&&run<"+runjec+"&&njets>=4", "trig[17]",
	     "With JEC bug 5.42fbMET120, N_{e,10}=0, N_{jet}#geq2, MET>200", "HT800_MET75",800,1,true,false);
...
TString PlotTurnOn(TChain *data, TString var, int nbins, double minx, double maxx, TString xtitle, 
  ...
  can.SetGrid();
  TH1D* histo[2];
  TString passcuts = "&&pass_hbhe&&pass_hbheiso&&pass_goodv&&pass_ecaldeadcell";
