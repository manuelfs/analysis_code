...
int main(){ 
  ...
  TString htcut, mhtcut, mucut;
  TString htti, mhtti;

  cout<<endl<<endl<<" ===== IsoMu muon ID ==== "<<endl;

  baseline = "(trig[28]||trig[33])&&njets>=2&&mht/met<5&&met/met_calo<5";
  title = "MET90, N_{jet}#geq2";
  htcut="&&ht>300&&ht<500"; mhtcut="&&mht>150"; mucut="&&Max$(mus_pt*(mus_sigid&&mus_miniso<0.2))>24";
  TString mupt="&&Max$(mus_pt*(mus_sigid&&mus_miniso<0.2))";
  htti=", 300<H_{T}<500"; mhtti=", H_{T}^{miss}>150";

  Efficiency(&c_met, baseline+mhtcut+htcut+mucut, "trig[32]");
  Efficiency(&c_met, baseline+mhtcut+htcut+mucut+mupt+"<50", "trig[32]||trig[21]");
  Efficiency(&c_met, baseline+mhtcut+htcut+mucut+mupt+">50", "trig[32]||trig[21]");

  minx = 10; maxx = 90; nbins = static_cast<int>((maxx-minx)/2);
  PlotTurnOn(&c_met, "Max$(mus_pt*(mus_sigid&&mus_miniso<0.2))", nbins,minx,maxx, "Medium muon p_{T}",
	     baseline+mhtcut+htcut, "(trig[21]||trig[32])",title+htti+mhtti, "IsoMu22 || Mu50",-24,1,true,false);
  ...
  TString e_isomu = Efficiency(&c_met, baseline+mhtcut+htcut+mucut, "trig[32]");
  TString e_isomu_lo = Efficiency(&c_met, baseline+mhtcut+htcut+mucut+"&&run<274094", "trig[32]");
  TString e_isomu_hi = Efficiency(&c_met, baseline+mhtcut+htcut+mucut+"&&run>=274094", "trig[32]");
  ...
  TString e_mu = Efficiency(&c_met, baseline+mhtcut+htcut+mucut, "trig[4]");
  TString e_mu_lo = Efficiency(&c_met, baseline+mhtcut+htcut+mucut+"&&run<274094", "trig[4]");
  TString e_mu_hi = Efficiency(&c_met, baseline+mhtcut+htcut+mucut+"&&run>=274094", "trig[4]");
