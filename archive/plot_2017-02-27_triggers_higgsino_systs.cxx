...
int main(){ 
  ...
  bool do_table = true;
  bool do_met = false, do_elel = false, do_mumu = false, do_el = false, do_mu = false;
  bool do_truemet = true;
  bool do_syst = false, do_mc = true;

  vector<TString> metbins_sys({"150", "160", "180", "200", "225", "250", "300", "9999"});
  vector<float> sys_uncert({0.071, 0.044, 0.042, 0.024, 0.015, 0.012, 0.006});

  if(do_mc){
    cuts = "nels==1&&njets>=3&&met/met_calo<5&&leps_pt[0]>30&&!low_dphi";
    title = "N_{j}#geq3, high #Delta#phi, N_{e}=1";

    minx = 150; maxx = 440; nbins = static_cast<int>((maxx-minx)/5);
  ...
  if(do_syst){
    TString outname = "effs_syst.txt";
    cuts = "trig[40]&&nels==1&&njets>=3&&met/met_calo<5&&leps_pt[0]>30&&!low_dphi";
    title = "Ele27, N_{j}#geq3, N_{e}=1";
    TString trigger = met_trig;

    vector<TString> syst_names({"Kinematic variations",
	  "Reference trigger (for $H_T>900$)", "Data compared to MC (MET120\\_MHT120 trigger only)"});
    vector<vector<TString> > varbins({{"njets>=3", "njets==4", "njets==5", "nbm==0", "nbm==1", "nbm>=2", 
	    "hig_am<=140", "hig_am>140", "hig_drmax<=2.2", "hig_drmax>2.2"},
	  {"trig[40]&&ht>900", "trig[54]&&ht>900"}, {"trig[40]", "1"}});

    float effic, errup, errdown;
    vector<vector<vector<vector<float> > > > effs;
    for(unsigned scen=0; scen<varbins.size(); scen++){
      if(scen>0) cuts.ReplaceAll("trig[40]&&", "");
      effs.push_back(vector<vector<vector<float> > >());
      for(unsigned imet=0; imet<metbins_sys.size()-1; imet++){
	effs[scen].push_back(vector<vector<float> >());
	for(unsigned ivar=0; ivar<varbins[scen].size(); ivar++){
	  pchain = &c_met;
	  if(scen==1 && ivar==1) {
	    pchain = &c_ht;
	  }
	  ...
	  if(scen==2 && ivar==1) {
	    pchain = &c_mc;
	    trigger = "(trig[3] || trig[7])";
	  } 
	  Efficiency(pchain, cuts+"&&"+varbins[scen][ivar]+"&&met"+">"+metbins_sys[imet]+"&&met"+"<="+metbins_sys[imet+1], 
		     trigger, effic, errup, errdown);
	  effs[scen][imet].push_back(vector<float>{effic,errup,errdown});
	} // vars
	cout<<endl;
      } // mets
      cout<<endl;
    ...
    cout<<endl<<endl;
    cout<<"\\begin{table}[tbp!]\n \\centering\n \\renewcommand{\\arraystretch}{1.2}\n";
    cout<<" \\resizebox{\\textwidth}{!}{\n \\begin{tabular}{l";
    for(unsigned imet=0; imet<metbins_sys.size()-1; imet++) cout<<"c";
    cout<<"}\n \\hline\\hline \n $E_{\\rm T}^{\\rm miss}$ range";
    for(unsigned imet=0; imet<metbins_sys.size()-1; imet++) {
      cout<<" & ["<<metbins_sys[imet];
      if(imet < metbins_sys.size()-2) cout<<","<<metbins_sys[imet+1];
      else cout<<"+";
      cout<<"]";
    } // mets
    cout<<"\\\\"<<endl; 
    vector<vector<float> > systs;
    for(unsigned scen=0; scen<varbins.size(); scen++){
      cout<<" \\hline \\hline \n \\multicolumn{"<<metbins_sys.size()<<"}{c}{"<<syst_names[scen]<<"}\\\\ \\hline \n";
      systs.push_back(vector<float>(metbins_sys.size(), 0));
      for(unsigned ivar=0; ivar<varbins[scen].size(); ivar++){
	if(scen==1) varbins[scen][ivar].ReplaceAll("&&ht>900","");
	if(scen==2 && ivar==0) cout<<setw(17)<<"Data";
	else if(scen==2 && ivar==1) cout<<setw(17)<<"MC";
	else cout<<setw(27)<<cuts2tex(varbins[scen][ivar]);
	if(ivar==0) cout<<" (nominal)";
	for(unsigned imet=0; imet<metbins_sys.size()-1; imet++){
	  cout<<" &  "<<"$"+RoundNumber(effs[scen][imet][ivar][0]*100,1)+"^{+"
	    +RoundNumber(effs[scen][imet][ivar][1]*100,1)+"}_{-"+RoundNumber(effs[scen][imet][ivar][2]*100,1)
	    +"}$ ";
	  // Calculating systematic as largest deviation with respect to nominal (ivar = 0)
	  if(fabs(effs[scen][imet][ivar][0] - effs[scen][imet][0][0]) > systs[scen][imet])
	    systs[scen][imet] = fabs(effs[scen][imet][ivar][0] - effs[scen][imet][0][0]);
	}
	cout<<"\\\\"<<endl;	
      }// vars
      cout<<" \\hline "<<endl;
      cout<<setw(17)<<"Systematic";
      for(unsigned imet=0; imet<metbins_sys.size()-1; imet++){
	systs[scen][imet] /= effs[scen][imet][0][0];
	cout<<" &  "<<setw(21)<<RoundNumber(systs[scen][imet]*100,1);
      }
      cout<<"\\\\"<<endl<<endl;
    }// scen
    cout<<"\\hline\\hline "<<endl;
    cout<<setw(17)<<"{\\bf Total systematic}";
    for(unsigned imet=0; imet<metbins_sys.size()-1; imet++){
      for(unsigned scen=1; scen<varbins.size(); scen++){
	systs[0][imet] = sqrt(pow(systs[0][imet],2) + pow(systs[scen][imet],2));
      } // scens
      cout<<" &  "<<setw(21)<<"{\\bf "<<RoundNumber(systs[0][imet]*100,1)<<"}";
    } // mets
    cout<<"\\\\ \\hline\\hline \n"<<" \\end{tabular} \n }\n\\end{table}"<<endl<<endl;
    
    cout<<endl<<"  vector<float> sys_uncert({";
    for(unsigned imet=0; imet<metbins_sys.size()-1; imet++){
      cout<<RoundNumber(systs[0][imet],3);
      if(imet<metbins_sys.size()-2) cout<<", ";
      else cout<<"});"<<endl;
    } // mets
    cout<<endl<<endl;
    outfile.close();
  ...
  if(do_met){
    ...
    if(do_truemet){
      cuts = "trig[40]&&nels==1&&njets>=3&&met/met_calo<5&&!low_dphi";
      title = "Ele27, N_{j}#geq3, high #Delta#phi, N_{e}=1";
      pchain = &c_met;
    } else {
	...
	for(unsigned ivar=0; ivar<varbins.size()-1; ivar++){
	  outfile<<"if("+var+">"<<setw(4)<<varbins[ivar]<<" && "+var+"<="<<setw(4)<<varbins[ivar+1]
		 ...
		 <<"; errup = "<<RoundNumber(effs[imet][ivar][1],3)<<"; errdown = "
		 <<RoundNumber(effs[imet][ivar][2],3)<<";}"<<endl;
	}
      }
      outfile.close();
      cout<<"Written "<<outname<<endl<<endl;
      outname = "effs_uncerts.txt";
      outfile.open(outname);
      for(unsigned imet=0; imet<metbins.size()-1; imet++){
	for(unsigned ivar=0; ivar<varbins.size()-1; ivar++){
	  float uncert = 0;
	  for(unsigned imets=0; imets<metbins_sys.size()-1; imets++){
	    if(metbins[imet].Atof()>=metbins_sys[imets].Atof() 
	       && metbins[imet].Atof()<metbins_sys[imets+1].Atof()){
	      uncert = sqrt(pow(max(effs[imet][ivar][1], effs[imet][ivar][2]),2) 
			    + pow(sys_uncert[imets],2));
	      break;
	    }
	  } // imets
	  outfile<<"if("+var+">"<<setw(4)<<varbins[ivar]<<" && "+var+"<="<<setw(4)<<varbins[ivar+1]
		 <<" && met>"<<setw(4)<<metbins[imet]<<" && met<="<<setw(4)<<metbins[imet+1]
		 <<") {uncert = "<<RoundNumber(uncert,3)
		 <<"; errup = "<<RoundNumber(effs[imet][ivar][1],3)<<"; errdown = "
		 <<RoundNumber(effs[imet][ivar][2],3)<<";}"<<endl;
	}
      }
    ...
    PlotTurnOn(pchain, "hig_am", nbins,minx,maxx, "#LTm#GT", cuts, met_trig,  title, trig_title,0);
